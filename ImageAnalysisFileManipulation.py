...
The order in which the conditions will be named in the output files for GraphPad analysis must be 
specified in the conditionOrder variable, which numbers the files based on alphabetical order
...
directory = "C:\\Users\\Sophie\\Desktop\\Test\\"
...
for subdir in os.listdir(directory):
        ...
        # Initialize the Master Summary for each condition by specifying the column names with the condtion and the parameter measured
        
        header = pd.DataFrame(['0','Index','Count','Area','Perim.','Circ.','AR','Round','Solidity','MTG','TMRE','Ratio','FormFactor','Total Area','Branches','Branch Length','Branch Points','Branches per Mito','Branch Length per Mito','Branch Points per Mito'])
        header = header.transpose()
        conditions = pd.DataFrame(['0',subdir,subdir,subdir,subdir,subdir,subdir,subdir,subdir,subdir,subdir,subdir,subdir,subdir,subdir,subdir,subdir,subdir,subdir,subdir])
        ...
        for file in os.listdir(directory + subdir + "\\Analysis"):
            ...
            morphologyMTGSummary = morphologySummary.merge(MTGSummary, on=' ')
            dataSummary = morphologyMTGSummary.merge(TMRESummary, on=' ')


            dataSummary.columns = ['Count','Area','Perimeter','Circularity','Aspect Ratio','Round','Solidity','MTG','TMRE']
            dataSummary.to_csv(directory + subdir + "\\Analysis\\" + file + "\\" + file + " Summary.csv", index=False)
                              
            # Calculate the averages, the ratio of TMRE to MTG, the Form Factor, the count number, and the total area
                   
            # Need new names
                   
            average = dataSummary.mean()
            Ratiovals = dataSummary['TMRE']/dataSummary['MTG']
            TMRE_MTGRatio = pd.Series(Ratiovals.mean(), index=['Ratio'])
            #TMRE_MTGRatio = TMRE_MTGRatio.transpose()
                       
            #TMRE_MTGRatio = pd.Series((average[8]/average[7])) #, index=["Ratio"]
            FFSeries = pd.Series(1/average[3]) #, index=['Form Factor']
            countSeries = pd.Series(len(dataSummary)) #, index=["Count"]
            sumSeries = dataSummary.sum()
            index = pd.Series([file]) #, index=['Index']
            branchSummary = branchSummary.transpose()
            
            finalSeries = index.append(countSeries)
            finalSeries = finalSeries.append(average[1:9])
            ...
            with open(directory + subdir + "\\Analysis\\" + subdir + " Summary " + name + ".csv", 'a') as f:
